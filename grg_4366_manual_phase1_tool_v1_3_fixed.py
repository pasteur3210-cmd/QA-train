...
def parse_qrcode(raw: str) -> Dict[str, str]:
    ...
    sn_patterns = [
        r"(?i)S/?N\s*[:=]\s*([A-Za-z0-9\-_]+?)(?=\s*(?:MAC|WPA|WIFI)\s*[:=]|$)",
        r"(?i)SN\s*[:=]\s*([A-Za-z0-9\-_]+?)(?=\s*(?:MAC|WPA|WIFI)\s*[:=]|$)",
    ]
    mac_patterns = [
        r"(?i)MAC\s*[:=]\s*([0-9A-Fa-f]{12})(?=\s*(?:WPA|WIFI|S/?N|SN)\s*[:=]|$)",
        r"(?i)MAC\s*[:=]\s*([0-9A-Fa-f:\-]{12,17})(?=\s*(?:WPA|WIFI|S/?N|SN)\s*[:=]|$)",
    ]
    wpa_patterns = [
        r"(?i)WPA(?:\s*KEY)?\s*[:=]\s*([^\s]+)",
...
class Phase1InspectionApp:
    ...
    def _build_right(self) -> None:
        ...
        bulk_btn_frame = tk.Frame(manual_box)
        bulk_btn_frame.grid(row=mrow, column=0, columnspan=2, sticky="we", pady=(8, 2))
        tk.Button(bulk_btn_frame, text="全部 PASS", width=14, height=1, command=self._mark_all_manual_pass).pack(side="left", padx=(0, 8))
        tk.Button(bulk_btn_frame, text="全部 N/A", width=14, height=1, command=self._mark_all_manual_na).pack(side="left")
