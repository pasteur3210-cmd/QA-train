...
def parse_device_sn(response: str, input_sn: str = "") -> str:
    ...
    candidates = []
    for tok in tokens:
        if tok.lower() in blacklist or tok.isdigit():
            continue
        if len(tok) >= 10:
...
def create_workbook(path: Path) -> None:
    wb = Workbook()
    headers = ["Time", "Input_SN", "Device_SN", "SN_Check_Result", "Host", "Config_Result", "Lamp_Check_Result", "Final_Result", "Matched", "Raw_Response", "Note"]
    ws_all = wb.active
    ws_all.title = "ALL"
...
def append_record(path: Path, record: Dict[str, str]) -> None:
    ...
    row = [sanitize_excel_text(record[k]) for k in ["time", "input_sn", "device_sn", "sn_check_result", "host", "config_result", "lamp_check_result", "final_result", "matched", "raw_response", "note"]]
    wb["ALL"].append(row)
...
def sn_exists(path: Path, sn: str) -> bool:
    ...
    try:
        for row in ws.iter_rows(min_row=2, values_only=True):
            if row and len(row) > 1 and str(row[1]).strip() == sn:
                return True
    finally:
        wb.close()
    return False
...
def run_setting(input_sn: str, host: str = HOST) -> Dict[str, object]:
...
class ProductionApp:
    ...
    def _build_ui(self) -> None:
        ...
        mid.pack(fill="x")
        tk.Label(mid, text="請刷入或輸入 SN", font=("Arial", 16, "bold")).grid(row=0, column=0, sticky="w", columnspan=6)
        self.sn_var = tk.StringVar()
        self.sn_entry = tk.Entry(mid, textvariable=self.sn_var, font=("Consolas", 18), width=34)
        self.sn_entry.grid(row=1, column=0, sticky="w", pady=(8, 8), columnspan=2)
        self.sn_entry.bind("<Return>", self._on_enter)
        self.start_btn = tk.Button(mid, text="測試開始", width=14, font=("Arial", 11, "bold"), command=self._start_test)
        self.start_btn.grid(row=1, column=2, padx=(12, 8), sticky="w")
        self.confirm_btn = tk.Button(mid, text="確認完成 / 換下一台", width=18, font=("Arial", 11, "bold"), command=self._confirm_next_unit, state="disabled")
        self.confirm_btn.grid(row=1, column=3, padx=(0, 8), sticky="w")
        self.lamp_fail_btn = tk.Button(mid, text="燈號失敗 / 記錄FAIL", width=18, font=("Arial", 11, "bold"), command=self._record_lamp_fail, state="disabled")
        self.lamp_fail_btn.grid(row=1, column=4, padx=(0, 8), sticky="w")
        self.exit_btn = tk.Button(mid, text="離開工具", width=12, font=("Arial", 11, "bold"), command=self._exit_tool)
        self.exit_btn.grid(row=1, column=5, padx=(0, 8), sticky="w")
        self.status_box = tk.Label(mid, text="等待掃碼", font=("Arial", 12, "bold"), width=58, height=2, bg="#d9d9d9", fg="black", relief="groove", anchor="w", padx=10)
        self.status_box.grid(row=2, column=0, columnspan=6, sticky="we", pady=(8, 4))
        ...
        self.log_text = tk.Text(bottom, height=18, font=("Consolas", 10))
    ...
    def _refresh_focus(self) -> None:
        self.root.after(100, lambda: self.sn_entry.focus_set())

    def _load_today_counts(self) -> None:
        if not self.excel_file.exists():
            self._update_counters(); return
        ...
        try:
            for row in ws.iter_rows(min_row=2, values_only=True):
                if not row:
                    continue
                self.total_count += 1
                result = str(row[7]).strip().upper() if len(row) > 7 and row[7] is not None else ""
                if result == "PASS":
                    self.pass_count += 1
                else:
                    self.fail_count += 1
                self.last_sn = str(row[1]).strip()
        finally:
            wb.close()
    ...
    def _play_sound(self, success: bool) -> None:
        try:
            import winsound
            if success:
                winsound.Beep(1600, 180); winsound.Beep(2000, 220)
            else:
                winsound.Beep(700, 350); winsound.Beep(500, 400)
        except Exception:
            pass
    ...
    def _worker(self, sn: str) -> None:
        self.result_queue.put(run_setting(input_sn=sn, host=self.host))
    ...
    def _poll_queue(self) -> None:
        try:
            ...
            self.root.after(120, self._poll_queue); return
        mode = result["mode"]
        ...
        if mode == "final":
            ...
            self._append_ui_log("請依現場燈號選擇：確認完成 / 換下一台，或 燈號失敗 / 記錄FAIL")
            self._set_busy(False)
            self._play_sound(True)
    ...
    def _start_test(self) -> None:
        ...
        self._set_busy(True)
        self._set_wait_buttons(False)
        threading.Thread(target=self._worker, args=(sn,), daemon=True).start()
